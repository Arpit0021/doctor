...
genai.configure(api_key="AIzaSyDhMC_PEi-3ueM7a6jVc1qZhxTQSfQd7ZU")  # Replace with your actual Gemini API key
model = genai.GenerativeModel('gemini-2.0-flash') # Using 'gemini-pro-vision' as 'gemini-2.0-flash' might not be a direct model name.

# Set Entrez email for NCBI API
Entrez.email = "arpit.rangra19ar@gmail.com"
...
async def analyze_image(image, api_key, enable_xai=True):
    """Analyze medical image using Gemini Pro Vision model for 100% insights"""
    ...
    try:
        response = await model.generate_content(
            ...
            generation_config=genai.types.GenerationConfig(max_output_tokens=1500) # Increase max tokens for more detailed output
        )
        response.resolve()
...
async def main():
    ...
    dummy_image_array = np.random.randint(0, 2)